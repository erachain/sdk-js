...
export class Bytes {
  ...
  static async intToByteArray2(int32: number): Promise<Int8Array> {
    const byteArray = [];
    for (let b = 16; b < 32; b += 8) {
  ...
  static syncStringToByteArray(str: string): Int8Array {
    const utf8 = [];
    if (str !== undefined) {
      for (let i = 0; i < str.length; i++) {
        let charcode: number = str.charCodeAt(i);
  ...
  static syncStringFromByteArray(bytes: Int8Array): string {
    const decoder = new StringDecoder('utf8');
    return decoder.end(Buffer.from(bytes));
  }
}
