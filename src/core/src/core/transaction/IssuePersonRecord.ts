...
export class IssuePersonRecord extends Issue_ItemRecord {
    ...
    public constructor(creator: PrivateKeyAccount, person: PersonCls, feePow: number, timestamp: number, reference: number, port: number) {
        super(new Int8Array([IssuePersonRecord.TYPE_ID, 0, 0, 0]), IssuePersonRecord.NAME_ID, creator, person, feePow, timestamp, reference, port);
    }
