...
export abstract class Transaction {
  ...
  protected static BASE_LENGTH =
    Transaction.TYPE_LENGTH +
    Transaction.FEE_POWER_LENGTH +
  ...
  protected static BASE_LENGTH_AS_PACK =
    Transaction.TYPE_LENGTH + Transaction.CREATOR_LENGTH + Transaction.SIGNATURE_LENGTH;
  ...
  async calcFee(): Promise<void> {
    const fee = new BigDecimal(await this.calcBaseFee()).setScale(8, BigDecimal.ROUND_UP);
