...
export class TransactionMessage extends Transaction {
    ...
    constructor(typeBytes: Int8Array, name: string, creator: PrivateKeyAccount, feePow: number, recipient: Account, timestamp: number, reference: number, head: string, message: Int8Array, encrypted: Int8Array, isText: Int8Array, port: number) {
    ...
    async getDataLength(includeReference: boolean): Promise<number> {
        return TransactionMessage.BASE_LENGTH
