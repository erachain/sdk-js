...
export class Template extends ItemCls {
  ...
  constructor(
    ...
    super(new Int8Array([1, 0]), owner, name, icon, image, description);
  }

  /* tslint:disable-next-line */
  static async parse(rowData: string, includeReference: boolean = false): Promise<Template> {
    let data: Int8Array; 
    ...
    //RETURN
    const template = new Template(owner, fullName, icon, image, description);

    template.typeBytes = typeBytes;
    ...
    if (ownerSignature) {
      template.ownerSignature = ownerSignature;
    }
