...
export const tranSend = async (recipient: ITranRecipient, keyPair: KeyPair, asset: ITranAsset, body: ITranMessage, port: number ): Promise<ITranRaw> => {
    ...
    try {
        ...
        const reference = 1;
        const tx = new R_Send(privateAccount, feePow, recipientAccount, asset.assetKey, asset.amount, body.head, messageBytes, isText, isEncripted, timestamp, reference, port);
